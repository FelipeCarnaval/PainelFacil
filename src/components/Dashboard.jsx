...
export default function Dashboard({ view, profiles, tableState, setTableState, onExport }) {
    ...
    <div className="dashboard">
      {view.kpis.length > 0 && (
        <div className="kpi-row">
          {view.kpis.map((k, i) => <KpiCard key={i} {...k} />)}
        </div>
      )}
      ...
      {charts.length > 0 && (
        <div className="chart-grid">
          <Suspense fallback={<div className="card chart-card"><div className="empty-chart">Carregando gráficos…</div></div>}>
            {charts.map((c, i) => <ChartWidget key={i} chart={c} />)}
          </Suspense>
        </div>
      )}
